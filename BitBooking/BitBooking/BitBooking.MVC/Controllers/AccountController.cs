...
namespace BitBooking.MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(string token)
        {
            if (token != null)
            {
                var acc = context.Tokens.First(x => x.Hash == token);
                if (acc == null) { return View(); }
                else
                {
                    ViewBag.Hotel = context.Accomodations.FirstOrDefault(x => x.AccomodationId == acc.AccomodationId);
                    RegisterViewModel model = new RegisterViewModel{AccomodationId=acc.AccomodationId};
        ...
        public async Task<ActionResult> RegisterAdmin(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser() { UserName = model.Email, Email = model.Email, AccomodationId = model.AccomodationId };
                IdentityResult result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    //Assign Role to user Here 
