...
namespace BitBooking.API.Controllers
{
    public class UploadPhotosController : ApiController
    {
        ...
        public void UploadFile(string RoomTypeId, string AccomodationId)
        {
                ...
                if (HttpContext.Current.Request.Files.AllKeys.Any())
                {
                    ...
                    if (File.Exists(fileSavePath))
                    {
                        ...
                        try
                        {
                            ...
                            bool parseSuccedRoom = Int32.TryParse(RoomTypeId,out roomTypeId);
                            if(parseSuccedAcc == true & parseSuccedRoom == true)
                            {
                                ...
                                newPhoto.RoomTypeId = roomTypeId;

                                db.Images.Add(newPhoto);
                                db.SaveChanges();
                            }
                            if (parseSuccedRoom == false)
                            {
                                string photoUrl = "http://res.cloudinary.com" + uploadResult.Uri.AbsolutePath;
                                Photo newPhoto = new Photo();
                                newPhoto.AccomodationId = accomodationID;
